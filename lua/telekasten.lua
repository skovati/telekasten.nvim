...
function Pinfo:resolve_link(title, opts)
    ...
    self.template = M.Cfg.note_type_templates.normal
    ...
    elseif opts.template_handling == "smart" then
        if self.is_daily then
            self.template = M.Cfg.note_type_templates.daily
        elseif self.is_weekly then
            self.template = M.Cfg.note_type_templates.weekly
        else
            self.template = M.Cfg.note_type_templates.normal
        end
    end
...
local function FindDailyNotes(opts)
    ...
    then
        create_note_from_template(today, _, fname, M.Cfg.note_type_templates.daily)
        opts.erase = true
...
local function FindWeeklyNotes(opts)
    ...
    then
        create_note_from_template(title, _, fname, M.Cfg.note_type_templates.weekly)
        opts.erase = true
        opts.erase_file = fname
    end
...
local function GotoDate(opts)
    ...
    then
        create_note_from_template(
            ...
            _,
            fname,
            M.Cfg.note_type_templates.daily,
            opts.dates
        )
        opts.erase = true
        opts.erase_file = fname
